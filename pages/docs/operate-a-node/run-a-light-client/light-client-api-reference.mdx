...
<Callout type="info">
BEFORE WE START
1. The individual behaviour of some methods may differ depending on which mode the light client is running in.
...
Gets the header for a specific block if available.
...
Although there are a variety of libraries we can use, the following examples are using [wscat](https://github.com/websockets/wscat).
