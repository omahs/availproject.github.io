...
1. Run the Avail [<ins>node</ins>](/docs/operate-a-node/run-a-full-node/full-node/0020-full-node-binaries). For local set-up, run the node in `dev` mode:

   ```bash
   ./data-avail --dev
   ```

2. Run a [<ins>bootstrap node</ins>](/docs/operate-a-node/run-a-full-node/0050-bootstrap-node).
...
4. If you want to supply your own configuration file (see [<ins>reference</ins>](https://github.com/availproject/avail-light#configuration-reference)), use:
   ```bash
...
We provide an Avail light client relay node that can help you with running your light client from behing NAT and firewalls. Find more information on running a relay node [<ins>here</ins>](/docs/operate-a-node/run-a-full-node/0060-relay-node).

### Run a Full Node

Consider running a full node by following the deployment guide [<ins>here</ins>](/docs/operate-a-node/run-a-full-node/full-node/0020-full-node-binaries).
