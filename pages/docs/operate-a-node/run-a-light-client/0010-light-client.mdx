...
run [`v1.7.8`](https://github.com/availproject/avail-light/releases/tag/v1.7.8) of the Avail light client for maximal network support. <br/>
...
2. **App-Specific Mode**: If an `app_id = 1` is given in the config file, the application client 
...
1. To spin up an Avail light client on the Goldberg testnet, run the following command in your terminal:

```sh
curl -sL1 avail.sh | sh -s -- --app_id 1 --network goldberg
```

2. You can pass additional flags to the script like:
...
- **app_id**: application ID to run the light client (defaults to 0). Set `app_id` to 1 to run in app-specific mode.
- **upgrade**: takes y and yes as valid arguments, indicating that the avail-light binary should be upgraded

3. If everything goes well, the client output will look like this:
...
<Tabs.Tab>
  ...
  1. Download the [recommended version of the binary](https://github.com/availproject/avail-light/releases/tag/v1.7.8) that is compatible with your operating system.
...
<Tabs.Tab>
   ...
   2. Make sure your terminal is in the `avail-light` directory. Create a compiled binary by running:
   ```sh
   cargo build --release
   ```

   3. This will create a new directory named `target` within `avail-light`. Navigate to `target/release` and you will find the `avail-light` binary.
...
```
   ...
   <Callout type="info">
   The exact '*binary type*' will differ from system to system. Take care to use the correct binary type for your system,
   and take care to use the correct name of the binary in you terminal.
   Alternatively, you could rename the binary to `avail-light` for convenience.
   </Callout>
